...
[plantuml, target=insert, format=svg, width=100%]
....
@startuml
skinparam defaultFontName JetBrains Mono
...
list -> tmp1 : save position
value -> list : put value in position

list -> list : increase list
list -> tmp2 : save position + 1
tmp1 -> list : set position

list -> list : increase list
list -> tmp1 : save postition + 2
tmp2 -> list : set position + 1
@enduml
....
